...
static int SDL_RenderGeometryRaw8BitColor(SDL_Renderer* renderer, ImVector<SDL_FColor>& colors_out, SDL_Texture* texture, const float* xy, int xy_stride, const SDL_Color* color, int color_stride, const float* uv, int uv_stride, int num_vertices, const void* indices, int num_indices, int size_indices)
{
    ...
    {
        // Use bit shifts to extract channels from ImU32 = (A<<24)|(B<<16)|(G<<8)|R,
        // which is endian-independent. SDL_Color member access is wrong on big-endian.
        ImU32 c = *(const ImU32*)color2;
        color3[i].r = (float)(c & 0xFF) / 255.0f;
        color3[i].g = (float)((c >> 8) & 0xFF) / 255.0f;
        color3[i].b = (float)((c >> 16) & 0xFF) / 255.0f;
        color3[i].a = (float)((c >> 24) & 0xFF) / 255.0f;
        color2 += color_stride;
    ...
    {
        SDL_BlendMode prev;
        SDL_GetTextureBlendMode(texture, &prev);
        SDL_SetTextureBlendMode(texture, SDL_BLENDMODE_BLEND);
        auto ret = SDL_RenderGeometryRaw(renderer, texture, xy, xy_stride, color3, sizeof(*color3), uv, uv_stride, num_vertices, indices, num_indices, size_indices);
        SDL_SetTextureBlendMode(texture, prev);
        return ret;
    }
    else
    {
...
void ImGui_ImplSDLRenderer3_RenderDrawData(ImDrawData* draw_data, SDL_Renderer* renderer)
{
    ...
    SDL_GetRenderDrawBlendMode(renderer, &old.BlendMode);
    ...
    SDL_SetRenderDrawBlendMode(renderer, old.BlendMode);
}
